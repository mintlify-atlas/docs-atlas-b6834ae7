...
<Steps>
  ...
  <Step title="Send the key on every captive-portal request">
    Pass the full key (including the `jwt_` prefix) in the `X-API-Key` header:
...
When the API receives a request with an `X-API-Key` header, `AuthHandler.authenticate_api_key` (in `app/core/auth.py`) performs the following checks in order:

1. **Prefix check** — The value must start with `jwt_`. Any other format returns `401 Formato de API Key inválido`.
